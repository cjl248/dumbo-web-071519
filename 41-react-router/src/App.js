...
export default class App extends React.Component {
    ...
    return (
      <div className="App">
        <header className="App-header">
          ...
          <aside className="sidebar">
            <ul>
              ...
              {
                this.state.spiceGirls.map(spice => (<li key={ spice.id }>
                  <NavLink to={ "/spice/" + spice.slug } exact activeClassName="selected">{ spice.adjective }</NavLink>
                </li>))
              }
            </ul>
          </aside>
        ...
        <Switch>
        {
          ...
          // this.state.spiceGirls.map(spice => <Route path={ "/" + spice.slug } component={ SpiceGirl } />)

        }
          <Route path="/chilis">
            <marquee>🌶🌶🌶🌶🌶🌶🌶🌶</marquee>
          </Route>
          <Route path="/spice/:slug" render={ this.renderSpiceGirlBasedOnUrlParameters } />
  ...
  renderSpiceGirlBasedOnUrlParameters = (renderParams) => {
    // console.log(renderParams.match)
    const slug = renderParams.match.params.slug

    const foundSpice = this.state.spiceGirls.find(spice => spice.slug === slug)
    if (foundSpice)
      return <SpiceGirl { ...foundSpice } />
    else
      return <NotFound />
  }
